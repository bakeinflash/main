...
#define FT_COMPONENT  trace_objs
    ...
    if ( slot )
    {
      ...
      while ( cur )
      {
        if ( cur == slot )
        {
          ...
          if ( slot->mygeneric.finalizer )
            slot->mygeneric.finalizer( slot );
  ...
  {
    /* finalize client-specific data */
    if ( size->mygeneric.finalizer )
      size->mygeneric.finalizer( size );
                ...
                FT_Driver  driver )
  {
    ...
    if ( face->mygeneric.finalizer )
      face->mygeneric.finalizer( face );
