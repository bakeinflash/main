// bakeinflash_avm2_jit.cpp	-- Julien Hamaide <julien.hamaide@gmail.com>	2008
...
#define var_scope jit_getarg( 2 )
		...
		do
		{
			...
			switch (opcode)
			{
				...
				case 0x2C:	// pushstring
				{
					int index;
					ip += read_vu30(index, &m_code[ip]);
					const char* val = m_abc->get_string(index).c_str();
				...
				case 0x30:	// pushscope
				{
					...
					//IF_VERBOSE_ACTION(myprintf("EX: pushscope\t %s\n", val.to_string()));
				...
				case 0x5D:	// findpropstrict
				{
					...
					//IF_VERBOSE_ACTION(myprintf("EX: findpropstrict\t %s, obj=%p\n", name, obj));
				...
				case 0x5E:	// findproperty, Search the scope stack for a property
				{
					...
					struct findproperty
					{
						static void call( const char* name, array<as_value> & scope, array<as_value> & stack )
						{
							...
							IF_VERBOSE_ACTION(myprintf("EX: findproperty\t %s, obj=%p\n", name, obj));
				...
				case 0x60:	// getlex, Find and get a property.
				{
					...
					struct getlex
					{
						static void call( const char* name, array<as_value> & scope, array<as_value> &stack )
						{
							...
							IF_VERBOSE_ACTION(myprintf("EX: getlex\t %s, value=%s\n", name, val.to_string()));
	...
	void as3function_def::compile_stack_resize( int count )
	{
#if _ENABLE_JIT==1
		jit_load( m_compiled_code, jit_this_pointer, jit_getarg( 1 ) );
