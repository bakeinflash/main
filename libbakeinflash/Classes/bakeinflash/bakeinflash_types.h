...
#ifndef BAKEINFLASH_TYPES_H
#define BAKEINFLASH_TYPES_H
...
#	define IF_VERBOSE_ACTION(exp) if (bakeinflash::get_verbose_action()) { exp; }
#	define IF_VERBOSE_PARSE(exp) if (bakeinflash::get_verbose_parse()) { exp; }
#	define IF_VERBOSE_DEBUG(exp) if (bakeinflash::get_verbose_debug()) { exp; }
#else
...
namespace bakeinflash
{
	...
	struct rgba
	{
		...
		void	set_rgba(double color)
		{
			Uint32 rgb = Uint32(color);
			m_r = (Uint8)(rgb >> 24) & 0xFF;
			m_g = (Uint8)(rgb >> 16) & 0xFF;
			m_b = (Uint8)(rgb >> 8) & 0xFF;
			m_a = (Uint8)rgb & 0xFF;
		}
