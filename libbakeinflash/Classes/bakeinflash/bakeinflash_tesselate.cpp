// bakeinflash_tesselate.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
// However, the current integration with bakeinflash is very hasty and
...
// produce meshes with too many verts, much like the old bakeinflash
// tesselator.
//
...
// bool bakeinflash_tesselate_dump_shape = false;//xxxxxxx
...
//				copy_points_into_array(&paths.back(), pp->m_verts);
				...
				for (int j = i + 1; j < s_path_parts.size(); j++)
				{
					...
					if (!pj->m_processed && pj->m_right_style == style && pj->m_closed && pj->m_verts.size() > 0)
					{
						paths.resize(paths.size() + 1);
...
// 				//xxxxxxxxx
				...
				ear_clip_triangulate::compute(&trilist, s_path_parts, i);
				pp->m_processed = true;
