// bakeinflash_disasm.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
#define COMPILE_DISASM 1
	...
	int read_s24(const uint8 *a)
	{
		int val = a[0] | a[1]<<8 | a[2]<<16;
		if (val >= 0x00800000)
		{
			val -= 0x01000000;
		}
	...
	// Disassemble one instruction to the log, AVM2
	{
		...
		do
		{
			...
			{
				myprintf(":	unknown opcode 0x%02X\n", opcode);
				ip++;
			}
	...
	// Disassemble one instruction to the log.
	{
		...
		if (info == NULL)
		{
			myprintf("<unknown>[0x%02X]", action_id);
		}
		else
		{
			myprintf("%-15s", info->m_instruction);
		...
		if (action_id & 0x80)
		{
			...
			if (fmt == ARG_HEX)
			{
				...
				{
					myprintf(" 0x%02X", instruction_data[3 + i]);
				}
				myprintf("\n");
			}
			else if (fmt == ARG_STR)
			{
				...
				for (int i = 0; i < length; i++)
				{
					myprintf("%c", instruction_data[3 + i]);
				}
				myprintf("\"\n");
			}
			else if (fmt == ARG_U8)
			...
			else if (fmt == ARG_DECL_DICT)
			{
				...
				for (int ct = 0; ct < count; ct++)
				{
					...
					while (instruction_data[3 + i])
					{
						...
						if (i >= length)
						{
							myprintf("<disasm error -- length exceeded>\n");
							break;
						}
