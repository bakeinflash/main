// bakeinflash_sound.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
namespace bakeinflash
{
	...
	// Load a DefineSound tag.
	{
		...
		IF_VERBOSE_PARSE(myprintf("define sound: ch=%d, format=%d, rate=%d, 16=%d, stereo=%d, ct=%d\n",
...
/*
	...
	struct start_sound_tag : public execute_tag
	{
		...
		void	execute(character* m)
		{
			if (s_sound_handler)
			{
				s_sound_handler->push_action(m_stop_playback, m_handler_id, m_loop_count);
			}
		}
	};
	...
	// Load a StartSound tag.
	{
		...
		if (sam)
		{
			...
			IF_VERBOSE_PARSE(myprintf("start_sound tag: id=%d, stop = %d, loop ct = %d\n",
		...
		else
		{
			if (s_sound_handler)
			{
				myprintf("start_sound_loader: sound_id %d is not defined\n", sound_id);
			}
		}
