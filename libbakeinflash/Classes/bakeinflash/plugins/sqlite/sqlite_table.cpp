// sqlite_table.cpp	-- Vitaly Alexeev <vitaly.alexeev@yahoo.com>	2009
...
namespace sqlite_plugin
{
	...
	bool	sqlite_table::get_member(const tu_string& name, as_value* val)
	{
		...
		if (as_object::get_member(name, val) == false)
		{
			...
			// try as field number
			double res;
			if (string_to_number(&res, name.c_str()))
			{
				int n = (int) res;
