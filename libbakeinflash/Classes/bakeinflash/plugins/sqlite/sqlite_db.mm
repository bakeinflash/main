...
namespace sqlite_plugin
{
	...
	{
		fn.result->set_as_object(new sqlite_db());
	}
	...
	{
		// copy to database from boundle to documents if need
		NSFileManager *fileManager = [NSFileManager defaultManager];
		NSError *error;
		NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
		NSString *documentsDirectory = [paths objectAtIndex:0];
		NSString *docPath = [documentsDirectory stringByAppendingPathComponent :[NSString stringWithUTF8String: dbfile]];
			
		if ([fileManager fileExistsAtPath :docPath] == NO)
		{
			NSString *resourcePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent :[NSString stringWithUTF8String: dbfile]];

			[fileManager copyItemAtPath :resourcePath toPath :docPath error:&error];
//			NSLog(@"sqlite: copy %@, error=%@", docPath, error);
		}
...
//		NSLog(@"sqlite: connection to %@", docPath);
		tu_string file_name = [docPath UTF8String];
		
		int rc = sqlite3_open_v2(file_name.c_str(), &m_db, read_only ? SQLITE_OPEN_READONLY : SQLITE_OPEN_READWRITE, vfs);
	  if (rc != SQLITE_OK)
		{
			myprintf("sqlite: %s, %s\n", file_name.c_str(), sqlite3_errmsg(m_db));
			sqlite3_close(m_db);
			return false;
		}
	...
	static void sqlite_func(sqlite3_context *context, int argc, sqlite3_value **argv)
	{
		...
		// keep alive
		smart_ptr<sqlite_db> this_ptr = ctx->m_this_ptr.get();
		smart_ptr<as_function> func = ctx->m_func.get();
