// sqlite_db.h	-- Vitaly Alexeev <vitaly.alexeev@yahoo.com>	2009
...
namespace sqlite_plugin
{
	...
	bool sqlite_db::connect(const char* dbfile, bool read_only, const char* vfs)
	{
		...
		tu_string file_name = get_full_url(get_workdir(), dbfile);
	...
	static void sqlite_func(sqlite3_context *context, int argc, sqlite3_value **argv)
	{
		...
		for (int i = 0; i < argc; i++)
		{
			...
			{
				case SQLITE_INTEGER:
					val.set_double((double) sqlite3_value_int64(argv[i]));
					break;
				case SQLITE_FLOAT:
