// bakeinflash_3ds_def.cpp	-- Vitaly Alexeev <tishka92@yahoo.com>	2007
...
namespace bakeinflash
{
	...
	x3ds_definition::x3ds_definition(const tu_string& url) : 
		m_file(NULL)
	{
		m_file = lib3ds_file_load(url.c_str());
		if (m_file == NULL)
		{
			static int n = 0;
			if (n < 1)
	...
	void x3ds_definition::clear_obj(Lib3dsNode* node)
	{
		for (Lib3dsNode* p = node; p != 0; p = p->next)
		{
			if (p->user.p)
			{
				...
				glDeleteLists(obj->glist_id, 1);
				glDeleteLists(obj->flist_id, 1);
				free(obj);
			}
	...
	//	if (infile == NULL)
		...
		if (im)
		{
			...
			unsigned char* copyData = im->m_data; //ilGetData();
			...
			tex->scale_x = (GLfloat)width /(GLfloat)tex_size;
			tex->scale_y = (GLfloat)height /(GLfloat)tex_size;
