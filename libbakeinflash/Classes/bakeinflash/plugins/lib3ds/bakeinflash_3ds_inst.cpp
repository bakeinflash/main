...
#include "bakeinflash_3ds_inst.h"
...
// bodies of the shaders
#include "bakeinflash_3ds_shader.h"
...
namespace bakeinflash
{
		...
		m_play_state(PLAY)
	{
		...
		builtin_member("mapMaterial", as_map_material);
		builtin_member("play", as_3d_play);
		builtin_member("stop", as_3d_stop);
		builtin_member("gotoAndStop", as_3d_goto_and_stop);
		builtin_member("gotoAndPlay", as_3d_goto_and_play);
		...
		case GL_FRAMEBUFFER_UNSUPPORTED_EXT:
			myprintf("FBO configuration unsupported\n");
			return;

		default:
			myprintf("FBO programmer error\n");
			return;
		}
	...
	bool	x3ds_instance::goto_frame(float frame)
	{
	...
	void	x3ds_instance::advance_actions()
	{
		on_event(event_id::ENTER_FRAME);
		if (m_play_state == PLAY)
		{
			goto_frame(m_current_frame + 1);
		}
	}

	bool	x3ds_instance::get_member(const tu_string& name, as_value* val)
	{
	...
	bool	x3ds_instance::set_member(const tu_string& name, const as_value& val)
	{
	...
	bool	x3ds_instance::upload_shaders(GLuint &program_id, GLuint &vshader_id, GLuint &fshader_id)
	{
		...
		if (!success)
		{
			...
			glGetShaderInfoLog(vshader_id, MAX_INFO_LOG_SIZE, NULL, infoLog);
			myprintf("Error in vertex shader compilation!\n");
			myprintf("Info log: %s\n", infoLog);
			return false;
		}
		...
		{
			GLchar infoLog[MAX_INFO_LOG_SIZE];
			glGetShaderInfoLog(fshader_id, MAX_INFO_LOG_SIZE, NULL, infoLog);
			myprintf("Error in fragment shader compilation!\n");
			myprintf("Info log: %s\n", infoLog);
			return false;
		}
		...
		{
			GLchar infoLog[MAX_INFO_LOG_SIZE];
			glGetProgramInfoLog(program_id, MAX_INFO_LOG_SIZE, NULL, infoLog);
			myprintf("Error in program linkage!\n");
			myprintf("Info log: %s\n", infoLog);
			return false;
		}
	...
	void	x3ds_instance::validate_shader()
	{
		if (m_needs_validation && (m_program_id != 0))
		{
			...
			{
				GLchar infoLog[MAX_INFO_LOG_SIZE];
				glGetProgramInfoLog(m_program_id, MAX_INFO_LOG_SIZE, NULL, infoLog);
				myprintf("Error in program validation!\n");
				myprintf("Info log: %s\n", infoLog);
			}
			else
			{
	...
	void	x3ds_instance::set_shader_args(GLfloat tex_p, GLfloat bump_p)
	{
		...
		uniformLoc = glGetUniformLocation(m_program_id, "texturePercent");
		if (uniformLoc != -1)
		{
			glUniform1f(uniformLoc, tex_p);
		}
		uniformLoc = glGetUniformLocation(m_program_id, "bumpPercent");
		if (uniformLoc != -1)
		{
			glUniform1f(uniformLoc, bump_p);
		}
		validate_shader();
	}
...
#undef SWAP_ROWS
	...
	void	x3ds_instance::display()
	{
		...
		GLint uniformLoc = glGetUniformLocation(m_program_id, "textureMap");
		if (uniformLoc != -1)
		{
			glUniform1i(uniformLoc, 0);
		}
		uniformLoc = glGetUniformLocation(m_program_id, "bumpMap");
		if (uniformLoc != -1)
		{
			glUniform1i(uniformLoc, 1);
		}
		uniformLoc = glGetUniformLocation(m_program_id, "shadowMap");
		if (uniformLoc != -1)
		{
	...
	{
		obj->tex_percent = 0.0f;
		obj->bump_percent = 0.0f;
		if (mat)
		{
			...
			if (mat->texture1_map.name)
			{
				if (m_def->m_texture.get(mat->texture1_map.name, &tex))
				{
					...
					obj->tex_percent = mat->texture1_map.percent;
				}
			}
			if (mat->bump_map.name)
			{
				if (m_def->m_bump.get(mat->bump_map.name, &tex))
				{
					...
					glBindTexture(GL_TEXTURE_2D, tex->tex_id);
					glActiveTexture(GL_TEXTURE0);
					obj->bump_percent = mat->bump_map.percent;
				}
			}
		}
		else
	...
	void x3ds_instance::bind_material(Lib3dsMaterial* mat, float U, float V)
	{
		if (mat)
		{
			...
			if (m_def->m_texture.get(mat->texture1_map.name, &tex))
			{
				float s = U * tex->scale_x;
				float t = tex->scale_y * (1 - V);
				glTexCoord2f(s, t);
			}
	...
	void x3ds_instance::create_mesh_flist(Lib3dsMesh* mesh, x3ds_object *obj)
	{
		...
		if (f == NULL)
		{
			myprintf("lib3ds: mesh has no faces\n");
			return;
		}
		...
		Lib3dsMaterial* mat = f->material[0] ? lib3ds_file_material_by_name(m_def->m_file, f->material) : NULL;		
	...
	void x3ds_instance::render_node(Lib3dsNode* node, bool full)
	{
		...
		if (node->type == LIB3DS_OBJECT_NODE)
		{
			...
			if (!node->user.p)	// generate x3ds_object
			{
				...
				obj->glist_id = glGenLists(1);
				glNewList(obj->glist_id, GL_COMPILE);
				create_mesh_glist(mesh);
				glEndList();

				obj->flist_id = glGenLists(1);
				glNewList(obj->flist_id, GL_COMPILE);
			...
			if (full)
			{
				set_shader_args(obj->tex_percent, obj->bump_percent);
				glCallList(obj->flist_id);
			}
			else
			{
				glCallList(obj->glist_id);
			}
			glPopMatrix();
		}
	}
	...
	void	x3ds_instance::update_material()
	{
		...
		if (aux_buffers < 2)
		{
			...
			{
				n++;
				myprintf("Your video card does not have AUX buffers, can't snapshot movieclips\n");
			}
			return;
		}
			...
			m_material.begin(); it != m_material.end(); ++it)
		{
			...
			if (ch)
			{
				...
				{
					myprintf("Can't snapshot _root movieclip, material '%s'\n", it->first.c_str());
					continue;
				}
