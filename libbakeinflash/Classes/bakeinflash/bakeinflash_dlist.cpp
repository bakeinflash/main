// bakeinflash_dlist.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
namespace bakeinflash
{
	...
	character*	display_list::get_character_at_depth(int depth)
	{
		...
		if (index != -1)
		{
			character*	ch = m_display_object_array[index].m_character.get();
	...
	void display_list::clear(int index)
	// Removing the character at get_display_object(index).
	{
		...
		remove_keypress_listener(di.m_character.get());

		set_alive(di.m_character, false);
	...
	{
		//IF_VERBOSE_DEBUG(myprintf("dl::add(%d, '%s')\n", depth, ch->get_name().c_str()));
...
//		{
//			char noname[20];
//			snprintf(noname, 20, "instance%d", ++m_instance_index);
//			ch->set_name(noname);
//		}
	...
	// the specified depth.
	{
//		IF_VERBOSE_DEBUG(myprintf("dl::move(%d)\n", depth));//xxxxx

		int	size = m_display_object_array.size();
		if (size <= 0)
		{
			// error.
			myprintf("error: move_display_object() -- no objects on display list\n");
			//			assert(0);
			return;
		}
		...
		{
			// error.
			// myprintf("error: move_display_object() -- can't find object at depth %d\n", depth);
			//			assert(0);
			return;
		}
		
		display_object_info&	di = m_display_object_array[index];
		character*	ch = di.m_character.get();
		if (ch->get_depth() != depth)
		{
			// error
			myprintf("error: move_display_object() -- no object at depth %d\n", depth);
			return;
		}
	...
	// character.
	{
//		IF_VERBOSE_DEBUG(myprintf("dl::replace(%d)\n", depth));//xxxxx
	...
	{
//		IF_VERBOSE_DEBUG(myprintf("dl::remove(%d)\n", depth));//xxxxx

		int	size = m_display_object_array.size();
		if (size <= 0)
		{
			// it's not error
			//myprintf("remove_display_object: no characters in display list\n");
			return;
		}
		
		int	index = find_display_index(depth);
		if (index < 0 || index >= size || get_character(index)->get_depth() != depth)
		{
			// it's not error
			// myprintf("remove_display_object: no character at depth %d\n", depth);
			return;
		}

		// loadMovie changes id, so do not check id
/*		
		...
		if (id != -1)
		{
			...
			for (;;)
			{
				...
				{
					// Didn't find a match!
					myprintf("remove_display_object: no character at depth %d with id %d\n", depth, id);
					return;
				}
				index++;
			}
...
*/
	...
	void display_list::add_keypress_listener(character* ch)
	{
		...
		{
			get_root()->m_keypress_listener.add(ch);
		}
	}
	...
	void display_list::remove_keypress_listener(character* ch)
	{
		if (ch)
		{
			get_root()->m_keypress_listener.remove(ch);
		}
	}
	...
	void	display_list::advance_dlist()
	// advance referenced characters.
	{
		// iterate through the copy
		array< smart_ptr<character> > ch;
		int n = m_display_object_array.size();
		ch.resize(n);
		for (int i = 0; i < n; i++)
		{
			ch[i] = m_display_object_array[i].m_character;
		}
	...
	void	display_list::advance_actions()
	// advance referenced characters.
	{
		// iterate through the copy
		array< smart_ptr<character> > ch;
		...
		for (int i = 0; i < n; i++)
		{
			if (ch[i] != NULL)
			{
				ch[i]->advance_actions();
			}
		}
	}
	
	void	display_list::advance_ctor()
	// advance referenced characters.
	{
		// iterate through the copy
		array< smart_ptr<character> > ch;
	...
	void	display_list::load_bitmaps(array < smart_ptr<bitmap_info> >*	m_bitmap_hash)
	{
		for (int i = 0; i < m_display_object_array.size(); i++)
		{
			character*	ch = m_display_object_array[i].m_character.get();
			if (ch)
			{
				ch->load_bitmaps(m_bitmap_hash);
			}
		}
	}
	...
	// are obscured by higher depths.
	{
		...
		//myprintf("number of objects to be drawn %i\n", m_display_object_array.size());
		for (int i = 0; i < m_display_object_array.size(); i++)
		{
...
/*			if (masked)
	...
	void display_list::dump(int tabs)
	{
		tu_string tab;
		...
		myprintf("%s*** displaylist ***\n", tab.c_str());
		for (int i = 0, n = size(); i < n; i++)
		{
			character*	ch = get_character(i);
			if (ch->get_name().size() > 0)
			{
				myprintf("%s%s\n", tab.c_str(), ch->get_name().c_str());
			}
			else
			{
				if (cast_to<sprite_instance>(ch))
				{
					myprintf("%s<noname movieclip>\n", tab.c_str());
				}
				else
				{
					myprintf("%s<noname>\n", tab.c_str());
	...
	void display_list::set_instance_name(character* ch)
	{
		if (ch->get_name().size() == 0)
		{
			char noname[20];
