...
#include "bakeinflash/bakeinflash_jit_opcode.h"
...
namespace bakeinflash
{
	...
	hash<int, Uint64> profiling_table;
	hash<int, int> profiling_table_count;
	void log_and_reset_profiling()
	{
		...
		{
			float f = (float) it.get_value()/1000.0f;
			a.push_back(f);
		}
		...
		for (int i = 0; i < a.size(); i++)
		{
			for (hash<int,Uint64>::iterator it = profiling_table.begin(); it != profiling_table.end(); ++it)
			{
				...
				if (a[i] == f)
				{
					myprintf("%02X: %.2f ms, ", it.get_key(), (float) it.get_value()/1000.0f);
					myprintf("%d\n", profiling_table_count[it.get_key()]);
				}
			}
		}

		profiling_table.clear();
		profiling_table_count.clear();
	}
#endif
		...
		)
	{
		...
		as_value	method;
		if (env->get_variable(method_name, &method, dummy_with_stack) == false)
		{
			return "";
		}
	...
	{
		IF_VERBOSE_PARSE(myprintf("tag %d: do_action_loader\n", tag_type));

		IF_VERBOSE_ACTION(myprintf("-------------- actions in frame %d\n", m->get_loading_frame()));
	...
	void	do_init_action_loader(stream* in, int tag_type, movie_definition_sub* m)
	{
		...
		IF_VERBOSE_PARSE(myprintf("  tag %d: do_init_action_loader\n", tag_type));

		IF_VERBOSE_ACTION(myprintf("  -- init actions for sprite %d\n", sprite_character_id));
	...
	void	action_buffer::read(stream* in)
	{
		...
		for (;;)
		{
			...
			IF_VERBOSE_ACTION(myprintf("%4d\t", pc); log_disasm(&(*m_buffer.get())[instruction_start]); );
	...
	// the same action_buffer, then we log an error and ignore it.
	{
		...
		{
			myprintf("error: process_decl_dict(%d, %d): decl_dict was already processed at %d\n",
				  start_pc,
				  stop_pc,
		...
		for (int ct = 0; ct < count; ct++)
		{
			...
			m_dictionary[ct] = new as_string((const char*) &buffer[3 + i]);
			...
			while (buffer[3 + i])
			{
				...
				if (i >= stop_pc)
				{
					myprintf("error: action buffer dict length exceeded\n");

					// Jam something into the remaining (invalid) entries.
					while (ct < count)
					{
						m_dictionary[ct] = new as_string("<invalid>");
						ct++;
					}
					return;
				}
...
//		env->set_local_frame_top(local_stack_top);
	...
	void action_buffer::enumerate(as_environment* env, as_object* obj)
	{
		...
		nullvalue.set_null();
		env->push(nullvalue);
		IF_VERBOSE_ACTION(myprintf("-------------- enumerate - push: NULL\n"));
	...
	// loads user defined class from DLL / shared library
	{
		...
		bakeinflash_module_init module_init = find_type_handler(classname);
		...
		if (module_init == NULL)
		{
			...
			module_init = (bakeinflash_module_init) lib->get_function("bakeinflash_module_init");
		}
	...
	// The is_function2 flag determines whether to use global or local registers.
	{
		...
		{
			compile(env, start_pc, exec_bytes, retval);
		}
		...
		if (m_compiled_code.is_valid())
		{
			try
			{
				m_compiled_code.call< array<as_value>&, vm_stack&, vm_stack&, as_value* >(env->m_local_register, *env, *env, retval );
			}
			catch( ... )
			{
		...
		for (int pc = start_pc; pc < stop_pc && !found_error; )
		{
			...
			if ((action_id & 0x80) == 0)
			{
				IF_VERBOSE_ACTION(myprintf("EX:\t"); log_disasm(&buffer[pc]));
				...
				case 0x15:	// substring
				{
					...
					base = iclamp(base, 0, str.size());

					// Truncate if necessary.
					size = imin(str.size() - base, size);
				...
				case 0x1C:	// get variable
				{
					...
					as_value variable;
					env->get_variable(var_string, &variable, with_stack);

					IF_VERBOSE_ACTION(
					if (variable.to_object() == NULL) 
					{
						myprintf("-------------- get var: %s=%s\n",
									  var_string.c_str(), variable.to_string());
					} 
					else
					{
						myprintf("-------------- get var: %s=%s at %p\n", var_string.c_str(), variable.to_string(), variable.to_object_interface());
					})
				...
				case 0x1D:	// set variable
				{
					...
					IF_VERBOSE_ACTION(myprintf("-------------- set var: %s \n",
				...
				// duplicateMovieClip(target:MovieClip, newname:String, depth:Number) : Void
				{
					...
					{
						as_value val;
						env->get_variable(env->top(2).to_string(), &val, with_stack);
				...
				case 0x27:	// start drag movie
				{
					...
					if (targetChar == NULL)
					{
						myprintf("error: start_drag of invalid target '%s'.\n",  env->top(0).to_string());
					}
				...
				case 0x3B:	// delete2
				{
					...
					as_value obj;
					env->get_variable_raw(varname, &obj, with_stack);
					...
					if (obj.is_undefined() == false)
					{
						...
						if (env->update_local(varname, as_value()))
						{
							// local var is found
				...
				case 0x3C:	// set local
				{
					IF_VERBOSE_ACTION(myprintf("-------------- set local: %s=%s\n", env->top(1).to_string(), env->top(0).to_string()));
				...
				case 0x3D:	// call function
				{
					...
					if (env->top(0).is_string())
					{
						...
						env->get_variable(function_name, &function, with_stack);
						...
						{
							myprintf("error in call_function: '%s' is not a function\n", function_name.c_str());
						}
					}
					else
					{
					...
					as_value	result = call_method(function, env, env->get_target(), nargs, env->get_top_index() - 2);
					env->drop(nargs + 1);
					env->top(0) = result;
					break;
				}
				...
				case 0x40:	// new
				{
					...
					IF_VERBOSE_ACTION(myprintf("-------------- new object: %s\n",
								  classname.to_tu_string().c_str()));
					int	nargs = env->pop().to_int();
					as_value constructor;
					env->get_variable(classname.to_tu_string(), &constructor, with_stack);
					as_value new_obj;
					...
					else
					{
						...
						{
							myprintf("can't create object with unknown class '%s'\n",
								  classname.to_tu_string().c_str());
						}
					}
...
//						obj->builtin_member(env->top(1).to_tu_string(), env->top(0));
				...
				{
					as_value variable;
					env->get_variable(env->top(0).to_tu_string(), &variable, with_stack);
...
//					// CHECK_STACK(2);
					...
					{
						const tu_string& s0 = env->top(0).to_tu_string();
						const tu_string& s1 = env->top(1).to_tu_string();
						if (s0.size() == s1.size() && s1.size() == 1)
						{
							env->top(1).set_bool(*s1.c_str() < *s0.c_str());
						}
						else
						{
							env->top(1).set_bool(s1 < s0);
						}
					}
					else
					{
				...
				case 0x49:	// equal (typed)
				{
					...
					bool test = env->top(1) == env->top(0);

					// null != 0
					if (test && ((env->top(0).is_number() && env->top(1).is_object()) || (env->top(1).is_number() && env->top(0).is_object())))
					{
						test = false;
					}

					env->top(1).set_bool(test);
					env->drop(1);
					break;
				}
				case 0x4A:	// to number
				...
				case 0x4E:	// get member
				{
					// CHECK_STACK(2);
					// keep alive 'obj'
					smart_ptr<as_object>	obj = env->top(1).to_object();
					if (obj == NULL)
					{
					...
					{
						// __resolve is not used.. so delete it for optimization
...
//						}
						...
						IF_VERBOSE_ACTION(
							...
							{
								myprintf("-------------- get_member %s=%s\n",
											env->top(0).to_tu_string().c_str(),
											env->top(1).to_tu_string().c_str());
							}
							else
							{
								myprintf("-------------- get_member %s=%s at %p\n",
											env->top(0).to_tu_string().c_str(),
...
//								env->top(2).to_tu_string().c_str(),
					...
					else
					{
						...
						IF_VERBOSE_ACTION(
							myprintf("-------------- set_member %s.%s=%s on invalid object!\n",
								env->top(2).to_tu_string().c_str(),
								env->top(1).to_tu_string().c_str(),
								env->top(0).to_tu_string().c_str()));
					}
				...
				case 0x52:	// call method
				{
					...
					const as_value& top = env->top(0);
					as_value func;
					if (top.is_undefined()) // == "undefined" || method_name == "")
					{
						// Flash help says:
						...
						{
							myprintf("error: undefined constructor %p\n", env->top(1).to_object());
						}
					}
					else
					{
						const tu_string&	method_name = top.to_tu_string();
						...
						else
						{
							...
							if (obj->get_member(method_name, &func))
							{
								result = call_method(
									func,
									env,
							...
							{
								//myprintf("error: can't find method %s\n", method_name.c_str());
							}
						}
					}
					env->drop(nargs + 2);
				...
				// The “undefined” return value should be popped off the stack.
					...
					{
						// C function is responsible for creating the new object and setting members.
						new_obj = call_method( constructor, env, env->get_target(), nargs, env->get_top_index()).to_object();
					}
					else if (cast_to<as_s_function>(constructor.to_object()))
					{
						new_obj = new as_object();
						as_object* proto = new_obj->create_proto(constructor);
						proto->m_this_ptr = new_obj.get();

						// Call the actual constructor function; new_obj is its 'this'.
						// We don't need the function result.
						call_method(constructor, env, new_obj.get(), nargs, env->get_top_index());
					}
					else
					{
						as_value val;
...
//								prototype.to_object()->set_member("__constructor__", func);
								...
								// Call the actual constructor function; new_obj is its 'this'.
								// We don't need the function result.
								call_method(func, env, new_obj.get(), nargs, env->get_top_index());
							}
							else 
							if (as_c_function* c_func = cast_to<as_c_function>(val.to_object()) )
							{
								// TODO: I don't get this prototype stuff, need to implement it
								
								// Result contains the new output.
								new_obj = call_method(c_func, env, env->get_target(), nargs, env->get_top_index()).to_object();
							}
							else
							{
					...
					// places new object to heap
					set_alive(new_obj.get());

					env->drop(nargs);
					env->push(new_obj.get());
					break;
				}
				...
				case 0x69:	// extends
					...
					if (sub.to_object() && super.is_function())
					{
			...
			else
			{
				...
				case 0x81:	// goto frame
				{
					...
					if (next_pc < stop_pc)
					{
						int	next_action_id = buffer[next_pc];
						if ((next_action_id & 0x80) == 0)
						{
							if (next_action_id == 0x06)	// //gotoAndPlay ?
							{
								next_pc++;
								env->get_target()->set_play_state(character::PLAY);
							}
							else
							if (next_action_id == 0x07)	// //gotoAndStop ?
							{
								next_pc++;
								env->get_target()->set_play_state(character::STOP);
							}
				...
				case 0x83:	// get url
				{
					...
					{
						// fscommand('setVolume', 100);
						sound_handler* s = get_sound_handler();
						if (s && strncmp(url + 10, "setVolume", 9) == 0)
						{
							s->set_max_volume(env->top(0).to_int());
						}
				...
				case 0x87:	// store_register
				{
					...
					else if (reg >= 0 && reg < 4)
					{
						...
						IF_VERBOSE_ACTION(
							myprintf("-------------- global register[%d] = '%s'\n",
								reg,
								env->top(0).to_string()));
					}
					else
					{
						myprintf("store_register[%d] -- register out of bounds!", reg);
					}
				...
				case 0x8E:	// function2
				{
					...
					if (function_exists == false)
					{
						...
						for (int n = 0; n < nargs; n++)
						{
							...
							i += func->m_args.back().m_name.size() + 1;
						}
				...
				{
					// CHECK_STACK(1);
					IF_VERBOSE_ACTION(myprintf("-------------- with block start: stack size is %d\n", with_stack.size()));
					...
					while (i - pc < length)
					{
						...
						switch (type)
						{
							case 0:
							{
								...
								IF_VERBOSE_ACTION(myprintf("-------------- pushed '%s'\n", str));
								break;
							}
							case 1:
							{
								...
								IF_VERBOSE_ACTION(myprintf("-------------- pushed '%f'\n", u.f));
								break;
							}
							case 2:
							{
								...
								IF_VERBOSE_ACTION(myprintf("-------------- pushed NULL\n"));
								break;
							}
							case 3:
							{
								...
								IF_VERBOSE_ACTION(myprintf("-------------- pushed UNDEFINED\n"));
								break;
							}
							case 4:
							{
								...
								if (is_function2)
								{
									...
									IF_VERBOSE_ACTION(myprintf("-------------- pushed local register[%d] = '%s'\n",	reg, env->top(0).to_string()));
								}
								else 
								if (reg < 0 || reg >= 4)
								{
									env->push(as_value());
									myprintf("push register[%d] -- register out of bounds!\n", reg);
								}
								else
								{
									env->push(env->m_global_register[reg]);
									IF_VERBOSE_ACTION(myprintf("-------------- pushed global register[%d], '%s', %p\n",	reg, env->top(0).to_string(),	env->top(0).to_object()));
								}
								break;
							}
							case 5:
	...
	//							}
								...
								IF_VERBOSE_ACTION(myprintf("-------------- pushed double %f\n", u.d));
								break;
							}
							case 7:
							{
								...
								IF_VERBOSE_ACTION(myprintf("-------------- pushed int32 %d\n", val));
								break;
							}
							case 8:
							{
								...
								{
									env->push(m_dictionary[id].get());

									IF_VERBOSE_ACTION(myprintf("-------------- pushed '%s'\n", m_dictionary[id]->to_string()));
								}
								else
								{
									myprintf("error: dict_lookup(%d) is out of bounds!\n", id);
									env->push(0);
									IF_VERBOSE_ACTION(myprintf("-------------- pushed 0\n"));
								}
								break;
							}
							case 9:
							...
							default:
								IF_VERBOSE_ACTION(myprintf("-------------- invalid push arg"));
								assert(0);
						}
					}
				...
				case 0x9A:	// get url2
				{
					...
					IF_VERBOSE_ACTION(myprintf("-------------- load URL2 %s\n", url));
					...
					if (strncmp(url, "print:", 6) == 0)
					{
						...
						as_value function;
						if (global->get_member("print", &function))
						{
							call_method(function, env, global, 1, env->get_top_index());
						}
					}
					else
					{
				...
				case 0x9D:	// branch if true
				{
					...
					if (test)
					{
						IF_VERBOSE_ACTION(myprintf("-------------- branch is done\n"));
						next_pc += offset;

						if (next_pc > stop_pc)
						{
							myprintf("branch to offset %d -- this section only runs to %d\n",
								  next_pc,
								  stop_pc);
						}
					}
					...
					{
						IF_VERBOSE_ACTION(myprintf("-------------- no branch\n"));
					}
					break;
				}
					...
					if (env->top(0).is_string())
					{
						...
						else
						{
							...
							if (string_to_number(&num, env->top(0).to_string()))
							{
								...
								success = target->goto_frame(frame_number);
							}
							// else no-op.
						}
					}
					else 
					if (env->top(0).is_number())
					{
						// Frame numbers appear to be 0-based!
			...
			if (profiling_table.find( action_id ) == profiling_table.end() )
			{
				...
				profiling_table_count.set( action_id, 0 );
			}
			profiling_table[action_id] += tu_timer::get_profile_ticks() - start_time;
			profiling_table_count[action_id]++;
#endif
		}
