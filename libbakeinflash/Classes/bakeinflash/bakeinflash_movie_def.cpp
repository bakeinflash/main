// bakeinflash_movie_def.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
#include "bakeinflash/bakeinflash_fontlib.h"
#include "base/jpeg.h"
...
*/
	...
	character_def*	movie_def_impl::get_character_def(int character_id)
	{
		...
		{
			myprintf("get_character_def(): character_id %d is still waiting to be imported\n",
				character_id);
		}
#endif // not NDEBUG
	...
	bool	movie_def_impl::get_labeled_frame(const tu_string& label, int* frame_number)
	...
	font*	movie_def_impl::get_font(int font_id)
	{
		...
		{
			myprintf("get_font(): font_id %d is still waiting to be imported\n",
				font_id);
		}
#endif // not NDEBUG
	...
	{
		for (hash<int, smart_ptr<font> >::const_iterator it = m_fonts.begin(); it != m_fonts.end(); ++it)
		{
...
//		add_bitmap_info(ch->get_bitmap_info());
	...
	bool	get_fileinfo(const char* url, int* width, int* height, int* fps)
	{
		...
		if (in.get_error() == TU_FILE_NO_ERROR)
		{
			...
			tu_file* m_zlib_in = &in;
			if (compressed)
			{
				m_zlib_in = zlib_adapter::make_inflater(&in);
			}

			stream* str = new stream(m_zlib_in);

			rect frame_size;
			frame_size.read(str);
			float frame_rate = str->read_u16() / 256.0f;

			*width = (int) TWIPS_TO_PIXELS(frame_size.m_x_max);
			*height = (int) TWIPS_TO_PIXELS(frame_size.m_y_max);
			*fps = (int) frame_rate;
	...
	// Read a .SWF movie.
	{
		...
		// do not check for embedded files
//		if ((header & 0x0FFFFFF) != 0x00535746 && (header & 0x0FFFFFF) != 0x00535743)
//		{
			// ERROR
//			myprintf("bakeinflash::movie_def_impl::read() -- file does not start with a SWF header!\n");
//			return;
//		}
		bool	compressed = (header & 255) == 'C';

		IF_VERBOSE_PARSE(myprintf("version = %d, file_length = %d\n", m_version, file_length));

		m_zlib_in = NULL;
		if (compressed)
		{
#if TU_CONFIG_LINK_TO_ZLIB == 0
			myprintf("movie_def_impl::read(): unable to read zipped SWF data; TU_CONFIG_LINK_TO_ZLIB is 0\n");
			return;
#endif

			IF_VERBOSE_PARSE(myprintf("file is compressed.\n"));
		...
		IF_VERBOSE_PARSE(myprintf("frame rate = %f, frames = %d\n", m_frame_rate, get_frame_count()));
	...
	void	movie_def_impl::read_tags()
	{
		...
		while ((Uint32) m_str->get_position() < m_file_end_pos && get_break_loading() == false)
		{
			...
			//IF_VERBOSE_PARSE(myprintf("tag_type = %d\n", tag_type));
			if (tag_type == 1)
			{
				// show frame tag -- advance to the next frame.
				IF_VERBOSE_PARSE(myprintf("  show_frame\n"));
				inc_loading_frame();
			}
			else
			...
			else
			{
				...
				myprintf("*** no tag loader for type %d\n", tag_type);
			...
			if (tag_type == 0)
			{
				if ((Uint32) m_str->get_position() != m_file_end_pos)
				{
					// Safety break, so we don't read past the end of the
					// movie.
					myprintf("warning: hit stream-end tag, but not at the "
	...
	// AVM2
	as_function* movie_def_impl::get_class_constructor(character* ch) const
	{
		...
		if (m_symbol_class.get(symbol_id, &class_name))
		{
			...
			return m_abc->create_class_constructor(class_name, ch);
		}
		return NULL;
	}
