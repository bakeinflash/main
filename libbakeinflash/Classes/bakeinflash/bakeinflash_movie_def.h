// bakeinflash_movie_def.h	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
#ifndef BAKEINFLASH_MOVIE_DEF_IMPL_H
#define BAKEINFLASH_MOVIE_DEF_IMPL_H
...
namespace bakeinflash
{
	...
	struct movie_definition : public character_def
	{
		...
		//
		// (optional) API to support bakeinflash::create_movie_no_recurse().
		//
		...
		//
		// Create the movie using bakeinflash::create_movie_no_recurse(..., DO_NOT_LOAD_BITMAPS),
		...
		// my_def = bakeinflash::create_movie_no_recurse("myfile.swf", DO_LOAD_BITMAPS);
		...
		// my_def = bakeinflash::create_movie_no_recurse("myfile.swf", DO_NOT_LOAD_BITMAPS);
		//
	...
	struct movie_definition_sub : public movie_definition 
	{
		...
		void	wait_frame(int frame)
		{
			if (is_multithread())
			{
				...
				{
//					myprintf("wait for frame %d, loaded %d\n", frame + 1, m_loading_frame);
					m_frame.wait();
				}
			}
		}
...
//				myprintf("signal for frame %d\n", m_loading_frame);
		...
		virtual bool	get_labeled_frame(const tu_string& label, int* frame_number) = 0;
...
//		virtual void	add_bitmap_info(bitmap_info* ch) = 0;
	...
	private:
		...
		void	signal()
		{
			// myprintf("signal %d frames is loaded\n", m_loading_frame);
			m_frame.signal();
		}
	...
	struct movie_def_impl : public movie_definition_sub
	{
		...
		string_hash<smart_ptr<character_def> > m_exports;
	...
	//unused ??	array<smart_ptr<bitmap_info> >	m_bitmap_list;
...
//		virtual bitmap_info*	get_bitmap_info(int i) const;
		...
		// AS3
		virtual as_function* get_class_constructor(character* ch) const;
		...
		const abc_def * get_abc() const{ return m_abc.get(); }
