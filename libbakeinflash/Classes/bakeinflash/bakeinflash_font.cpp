// bakeinflash_font.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003

// This source code has been donated to the Public Domain.  Do
// whatever you want with it.

// A font type for bakeinflash.
...
namespace bakeinflash
{
	...
	void	font::read(stream* in, int tag_type, movie_definition_sub* m)
	{
		...
		if (tag_type == 10)
		{
			IF_VERBOSE_PARSE(myprintf("reading DefineFont\n"));
			...
			IF_VERBOSE_PARSE(myprintf("offset[0] = %d\n", offsets[0]));
			int	count = offsets[0] >> 1;
			for (int i = 1; i < count; i++)
			{
				offsets.push_back(in->read_u16());
				IF_VERBOSE_PARSE(myprintf("offset[%d] = %d\n", i, offsets[i]));
			}
	...
	// codes.
	{
		IF_VERBOSE_PARSE(myprintf("reading code table at offset %d\n", in->get_position()));
...
*/
	...
	{
		return glyph_index < m_glyphs.size() ? m_glyphs[glyph_index].get() : NULL;
	}
