// bakeinflash_sprite_def.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
#ifndef BAKEINFLASH_SPRITE_DEF_H
#define BAKEINFLASH_SPRITE_DEF_H
...
namespace bakeinflash
{
	...
	struct sprite_definition : public movie_definition_sub
	{
		...
		weak_ptr<movie_definition_sub>	     m_movie_def;		// parent movie.
		...
		virtual void	add_character(int id, character_def* ch) { myprintf("add_character tag appears in sprite tags!\n"); }
		virtual void	add_font(int id, font* ch) { myprintf("add_font tag appears in sprite tags!\n"); }
		...
		virtual void	add_bitmap_character(int id, bitmap_character_def* ch) { myprintf("add_bc appears in sprite tags!\n"); }
		virtual sound_sample*	get_sound_sample(int id) { return m_movie_def->get_sound_sample(id); }
		virtual void	add_sound_sample(int id, sound_sample* sam) { myprintf("add sam appears in sprite tags!\n"); }
...
//		virtual int	get_bitmap_info_count() const { assert(0); return 0; }
	...
	//	virtual void	add_bitmap_info(bitmap_info* bi) { assert(0); }
		...
		virtual void	export_resource(const tu_string& symbol, character_def* res) { myprintf("can't export from sprite\n"); }
		...
		virtual void	add_init_action(int sprite_id, execute_tag* c)
		{
			...
			myprintf("sprite_definition::add_init_action called!  Ignored.\n");
		}
			...
			// kept in this object.
		{
			...
			{
				myprintf("add_frame_name(%d, '%s') -- frame name already assigned to frame %d; overriding\n",
					get_loading_frame(),
					name,
					currently_assigned);
			}
		...
		/* sprite_definition */
		bool	get_labeled_frame(const tu_string& label, int* frame_number)
