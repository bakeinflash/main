// array.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003, Vitaly Alexeev <vitaly.alexeev@yahoo.com>	2007
...
namespace bakeinflash
{
	...
	void	as_array_splice(const fn_call& fn)
	{
		...
		if (a && fn.nargs >= 1)
		{
			...
			if (index >= 0 && index < a->size())
			{
				...
				as_array* deleted_items = new as_array();
	...
	as_global_array::as_global_array() :
		as_c_function(as_global_array_ctor)
	{
	...
	const tu_string&	as_array::to_tu_string()
	{
		to_string();
	...
	bool	as_array::get_member(const tu_string& name, as_value* val)
	{
		int64 index;
		bool is_number = string_to_number(&index, name.c_str(), 10);

		// sanity check
		if (index < 100000 && is_number)
		{
			if (index >= 0 && index < size())
			{
				*val = m_array[(int) index];
				return true;
			}
		}
	...
	bool	as_array::set_member(const tu_string& name, const as_value& val)
	{
		int64 index;
		bool is_number = string_to_number(&index, name.c_str(), 10);
...
//		visited_objects->set(this, true);
	...
	void as_array::dump(int tabs)
	{
		for (int i = 0; i < m_array.size(); i++)
		{
			m_array[i].print(tu_string(i).c_str(), tabs + 2);
		}
