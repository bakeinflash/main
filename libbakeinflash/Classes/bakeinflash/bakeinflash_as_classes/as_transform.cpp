...
namespace bakeinflash
{
	...
	static string_hash<as_transform_member>	s_transform_member_map;
	static as_transform_member   get_transform_member(const tu_string& name);
	...
	bool	as_transform::set_member(const tu_string& name, const as_value& val)
	{
	...
	bool	as_transform::get_member(const tu_string& name, as_value* val)
	{
		as_transform_member	member = get_transform_member( name );
		...
		switch( member )
		{
			case colorTransform:
			{
				val->set_as_object( m_color_transform.get() );
				return true;
			}
			case concatenatedColorTransform:
			{
	...
	as_transform_member	get_transform_member(const tu_string& name)
	{
