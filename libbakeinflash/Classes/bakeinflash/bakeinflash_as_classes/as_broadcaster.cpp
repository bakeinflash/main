// as_broadcaster.cpp	-- Vitaly Alexeev <vitaly.alexeev@yahoo.com>	2007
...
namespace bakeinflash
{
	...
	bool	as_listener::get_member(const tu_string& name, as_value* val)
	{
		if (name == "length")
		{
	...
	void	as_listener::broadcast(const fn_call& fn)
	{
		...
		while (m_suspended_event.size() > 0)
		{
			...
			int nargs = arg.size() - 1;
			for (int j = nargs; j > 0; j--)
			{
				fn.env->push(arg[j]);
			}
				
			m_listeners.notify(event_name, fn_call(NULL, 0, fn.env, nargs, fn.env->get_top_index()));
