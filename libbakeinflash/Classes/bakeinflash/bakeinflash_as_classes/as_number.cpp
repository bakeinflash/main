// as_number.cpp	-- Vitaly Alexeev <vitaly.alexeev@yahoo.com>	2008
...
namespace bakeinflash
{
	...
	void as_global_parse_int(const fn_call& fn)
	{
		if (fn.nargs > 0)
		{
			...
			if (strncmp(fn.arg(0).to_string(), "0x", 2) == 0)
			{
				base = 16;
				if (string_to_number(&res, fn.arg(0).to_string() + 2, base))
				{
					fn.result->set_double((double) res);
				}
			}
			else
			{
