...
namespace bakeinflash
{
	...
	void	sound_start(const fn_call& fn)
	{
		...
		if (snd->iOS_sound)
		{
			...
			[audioPlayer play];
	//		printf("audioPlayer start: loops=%d %p\n", loops, audioPlayer);
#endif
		}
		else
		{
	...
	void	sound_get_volume(const fn_call& fn)
	{
		as_sound*	snd = cast_to<as_sound>(fn.this_ptr);
		if (snd == NULL)
		{
		...
		{			
			...
			if (s != NULL)
			{
				vol = s->get_volume(snd->m_id);
			}
		}
		fn.result->set_int(vol);
	}
	
	void	sound_set_volume(const fn_call& fn)
	{
		if (fn.nargs < 1)
		{
		...
		if (volume >= 0 && volume <= 100)
		{
			...
			//	printf("audioPlayer set volume: vol=%f %p\n", volume / 100.0f, audioPlayer);
#endif
			}
			else
			{			
		...
		{
			myprintf("audioPlayer: inavlid set volume: vol=%d\n", volume);
		}
...
//			NSLog(@"%@", [NSString stringWithFormat:@"%@/%@", [[NSBundle mainBundle] resourcePath], [NSString stringWithUTF8String: fn.arg(0).to_tu_string()]]);
	...
	void	as_global_sound_ctor(const fn_call& fn)
	{
		...
		snd->builtin_member("getVolume", sound_get_volume);
		snd->builtin_member("setVolume", sound_set_volume);
