...
namespace bakeinflash
{
  ...
  void	as_webview_fullscreen(const fn_call& fn)
  {
    as_iwebview* wv = cast_to<as_iwebview>(fn.this_ptr);
    if (wv)
    {
      wv->setFullScreen();
    }
...
//		www.autoresizingMask = (UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth);
    www.opaque = NO;
    www.scrollView.bounces = NO;
    www.scrollView.scrollEnabled = NO;
    www.backgroundColor = [UIColor clearColor];
    www.scalesPageToFit = true;
    www.contentMode = UIViewContentModeScaleAspectFit;
		[www setDelegate :www];
...
#endif
    set_member("_visible", false);
    builtin_member("setFullScreen", as_webview_fullscreen);
  ...
  void as_iwebview::setFullScreen()
  {
    if (m_iwebview)
    {
#ifdef iOS
...
#ifdef iOS
        myWebView* www = (myWebView*) m_iwebview;
        val->set_double(www.frame.size.width);
#endif
...
#ifdef iOS
          myWebView* www = (myWebView*) m_iwebview;
          val->set_double(www.frame.size.height);
#endif
...
#ifdef iOS
            myWebView* www = (myWebView*) m_iwebview;
            val->set_double(www.frame.origin.x);
#endif
...
#ifdef iOS
              myWebView* www = (myWebView*) m_iwebview;
              val->set_double(www.frame.origin.y);
#endif
	...
	bool	as_iwebview::set_member(const tu_string& name, const as_value& val)
	{
		if (name == "url")
		{
			...
			set_member("_visible", val.to_tu_string().size() > 0);
			return true;
		}
		else
		if (name == "_visible")
    ...
    if (name == "width")
    {
      if (m_iwebview)
      {
        ...
        CGRect rect = www.frame;
        rect.size.width = val.to_number();
        www.frame = rect;
    ...
    if (name == "height")
    {
      if (m_iwebview)
      {
        ...
        CGRect rect = www.frame;
        rect.size.height = val.to_number();
        www.frame = rect;
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
  ...
  // notify
	...
	[s_view sendMessage :"onLoadURL" :[s UTF8String]];
...
- (void)webViewDidFinishLoad:(UIWebView *)theWebView
{
 ...
 // theWebView.scrollView.zoomScale = rw;
  ...
  // notify
  NSString* s = theWebView.request.URL.absoluteString;
  [s_view sendMessage :"onLoadedURL" :[s UTF8String]];
