...
namespace bakeinflash
{
	...
	// called on them when a key changes state.
	{
		...
		if (fn.nargs < 1)
		{
			myprintf("key_add_listener needs one argument (the listener object)\n");
			return;
		}
		...
		if (listener == NULL)
		{
			myprintf("key_add_listener passed a NULL object; ignored\n");
			return;
		}
	...
	// Remove a previously-added listener.
	{
		...
		if (fn.nargs < 1)
		{
			myprintf("key_remove_listener needs one argument (the listener object)\n");
			return;
		}
	...
	// Return true if the specified (first arg keycode) key is pressed.
	{
		if (fn.nargs < 1)
		{
			myprintf("key_is_down needs one argument (the key code)\n");
			return;
		}
	...
	void	as_key::set_key_down(int code, Uint16 utf16char)
	{
		if (code < 0 || code >= key::KEYCOUNT) return;

		m_last_key_pressed = code;
		m_last_utf16_key_pressed = utf16char;
	...
	void	as_key::set_key_up(int code, Uint16 utf16char)
	{
		if (code < 0 || code >= key::KEYCOUNT) return;
	...
	int	as_key::get_last_utf16_key_pressed() const
	{
		return m_last_utf16_key_pressed;
	}
