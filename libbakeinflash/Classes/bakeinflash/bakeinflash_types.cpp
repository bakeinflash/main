// bakeinflash_types.h	-- Thatcher Ulrich <tu@tulrich.com> 2003

// This source code has been donated to the Public Domain.  Do
// whatever you want with it.

// Some basic types for bakeinflash.
...
namespace bakeinflash
{
	...
	// original xform.
	{
		float m00 = m_[0][0] * m.m_[0][0] + m_[0][1] * m.m_[1][0];
		float m10 = m_[1][0] * m.m_[0][0] + m_[1][1] * m.m_[1][0];
		float m01 = m_[0][0] * m.m_[0][1] + m_[0][1] * m.m_[1][1];
		float m11 = m_[1][0] * m.m_[0][1] + m_[1][1] * m.m_[1][1];
		float m02 = m_[0][0] * m.m_[0][2] + m_[0][1] * m.m_[1][2] + m_[0][2];
		float m12 = m_[1][0] * m.m_[0][2] + m_[1][1] * m.m_[1][2] + m_[1][2];
	...
	// Initialize from the stream.
	{
		...
		//IF_VERBOSE_PARSE(myprintf("  mat: has_scale = %d, has_rotate = %d\n", has_scale, has_rotate));
	}
	...
	// errors in tesselation.
	{
		...
		float	max_length2 = (float) fmax(basis0_length2, basis1_length2);
	...
	void matrix::fliph()	// top<==>bottom
	{
		m_[1][1] = -m_[1][1];
	}
	
	void matrix::flipv()	//	left <==> right
	{
		m_[0][0] = -m_[0][0];
	}
	
	void	matrix::concatenate_xyscale(float xs, float ys)
	// Concatenate a uniform scale onto the front of our
	// matrix.  When transforming points, the scale
	// happens first, then our original xform.
	{
...
//		IF_DEBUG(myprintf("rect::read() nbits = %d\n", nbits));
	}

	void	rect::print() const
	// Debug spew.
	{
		myprintf("xmin = %g, ymin = %g, xmax = %g, ymax = %g\n",
	...
	{
		m_x_min = (float) fmin(m_x_min, x);
		m_y_min = (float) fmin(m_y_min, y);
		m_x_max = (float) fmax(m_x_max, x);
		m_y_max = (float) fmax(m_y_max, y);
	}
