// bakeinflash_stream.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
//		IF_DEBUG(myprintf("stream::read_sint(%d) == %d\n", bitcount, value));
	...
	{
		align();
//		IF_DEBUG(myprintf("filepos = %d ", SDL_RWtell(m_input)));
		int	val = m_input->read_le16();
//		IF_DEBUG(myprintf("val = 0x%X\n", val));
		return val;
	}
...
//???		align();
	...
	// Return the tag type.
	{
		...
		IF_VERBOSE_PARSE(myprintf("---------------tag type = %d, tag length = %d\n", tag_type, tag_length));
	...
	// Seek to the end of the most-recently-opened tag.
	{
		...
		{
			IF_VERBOSE_PARSE( myprintf( "tag is not correctly read, tag length is not respected\n" ) );
		}
