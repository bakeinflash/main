// bakeinflash_render_handler_ogl.cpp	-- Willem Kokke <willem@mindparity.com> 2003

// This source code has been donated to the Public Domain.  Do
// whatever you want with it.

// A bakeinflash::render_handler that uses SDL & OpenGL

#include "base/tu_config.h"

#ifdef TU_USE_SDL
#include <SDL/SDL.h>  // for cursor handling & the scanning for extensions.
#include <SDL/SDL_opengl.h>	// for opengl const
#else
...
struct sdl_cursor_handler {
	...
	void set_cursor(bakeinflash::render_handler::cursor_type cursor)
	{
		if (m_inited == false)
		{
			init();
		...
		switch (cursor)
		{
			case bakeinflash::render_handler::SYSTEM_CURSOR:
				if (m_system_cursor)
				{
					SDL_SetCursor(m_system_cursor);
				}
			...
			case bakeinflash::render_handler::ACTIVE_CURSOR:
				if (m_active_cursor)
				{
			...
			case bakeinflash::render_handler::INVISIBLE_CURSOR:
				SDL_ShowCursor(SDL_DISABLE);
				break;

			case bakeinflash::render_handler::VISIBLE_CURSOR:
...
struct video_handler_ogl : public bakeinflash::video_handler
{
	GLuint m_texture;
	float m_scoord;
	float m_tcoord;
...
struct render_handler_ogl : public bakeinflash::render_handler
{
	// Some renderer state.
...
bakeinflash::render_handler*	bakeinflash::create_render_handler_ogl()
// Factory.
{
