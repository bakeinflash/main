// bakeinflash_character.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
namespace bakeinflash
{
	...
	bool	character::get_member(const tu_string& name, as_value* val)
	...
	// Otherwise leave *val alone and return false.
	{
		...
		switch (std_member)
		{
			...
			case M_XSCALE:
			{
				...
				val->set_double(get_root()->is_as3() ? xscale : xscale * 100);		// result in percent in AS2
				return true;
			}
			case M_YSCALE:
			{
				...
				val->set_double(get_root()->is_as3() ? yscale : yscale * 100);		// result in percent in AS2
				return true;
			}
			case M_ALPHA:
			{
			...
			case MDOT2:
			{
				val->set_as_object(cast_to<as_object>(m_parent.get()));
				return true;
			}
		}	// end switch
	...
	// TODO: call_watcher
	bool	character::set_member(const tu_string& name, const as_value& val)
	{
		// first try character members
		as_standard_member	std_member = get_standard_member(name);
		switch (std_member)
		{
			...
			case M_XSCALE:
			{
				if (!isnan( val.to_number() ) )
				{
					...
					float	x_scale = get_root()->is_as3() ? val.to_float() : val.to_float() / 100.f;	// input is in percent in AS2
					float	y_scale = m.get_y_scale();
					float	rotation = m.get_rotation();

					if (!isfinitef(x_scale)) x_scale = 0.0f;
					if (!isfinitef(y_scale)) y_scale = 0.0f;
					if (!isfinitef(rotation)) rotation = 0.0f;
			...
			case M_YSCALE:
			{
				if (!isnan( val.to_number() ) )
				{
					...
					float	y_scale = get_root()->is_as3() ? val.to_float() : val.to_float() / 100.f;	// input is in percent
					float	rotation = m.get_rotation();
			...
			case M_WIDTH:
			{
				if ( !isnan(val.to_number()) && val.to_float() >= 0)
				{
					matrix	m = get_matrix();
					...
					// set new scale
					x_scale /= current_width / (val.to_float() == 0 ? 1 : val.to_float());
			...
			case M_HEIGHT:
			{
				if ( !isnan(val.to_number()) && val.to_float() >= 0)
				{
					...
					// set new scale
					y_scale /= current_height / (val.to_float() == 0 ? 1 : val.to_float());
	...
	void	character::set_name(const tu_string& name)
	{
		m_name = name; 
		m_name.compute_hash();
	}

	const tu_string&	character::get_name() const
	{
		return m_name; 
	}

	void character::get_world_matrix(matrix* m) const
	...
	{
		const character* parent = m_parent.get();
		const matrix& this_m = get_matrix();
		if (parent)
		{
			parent->get_world_matrix(m);
			m->concatenate(this_m);
			return;
		}
		*m = this_m;
	}

	void character::get_world_cxform(cxform* m) const
	...
	{
		const character* parent = m_parent.get();
		const cxform& this_m = get_cxform();
		if (parent)
		{
			parent->get_world_cxform(m);
			m->concatenate(this_m);
			return;
		}
		*m = this_m;
