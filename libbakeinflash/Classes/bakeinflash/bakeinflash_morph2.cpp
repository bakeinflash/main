// bakeinflash_morph2.cpp
// -- Thatcher Ulrich <tu@tulrich.com>, Mike Shaver <shaver@off.net> 2003, Vitalij Alexeev <vitaly.alexeev@mail.ru> 2004.

// This source code has been donated to the Public Domain.  Do
// whatever you want with it.

// Loading and rendering of morphing shapes using bakeinflash_shape.
...
namespace bakeinflash
{
	...
	void	morph2_character_def::display(character* inst)
	{
		...
		m_mesh->display(mat, cx, m_fill_styles, m_line_styles, render_handler::BLEND_NORMAL, inst->get_rgba());
	}
	...
	void	morph2_character_def::read(stream* in, int tag_type, bool with_style, movie_definition_sub* md)
	{
		...
		for (i = 0; i < fill_style_count; i++)
		{
			...
			IF_VERBOSE_PARSE(myprintf("morph fill style type = 0x%X\n", fs1.m_type));
			...
			if (fs1.m_type == 0x00)
			{
				...
				IF_VERBOSE_PARSE(myprintf("morph fill style begin color: "); fs1.m_color.print());
				IF_VERBOSE_PARSE(myprintf("morph fill style end color: "); fs2.m_color.print());
			}
			else
			if (fs1.m_type == 0x10 || fs1.m_type == 0x12)
			{
				...
				IF_VERBOSE_PARSE(myprintf("morph fsr: num_gradients = %d\n", num_gradients));
			...
			if (fs1.m_type == 0x40 || fs1.m_type == 0x41)
			{
				...
				IF_VERBOSE_PARSE(myprintf("morph fsr bitmap_char = %d\n", bitmap_char_id));
