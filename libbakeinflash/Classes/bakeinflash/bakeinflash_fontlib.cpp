// bakeinflash_fontlib.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003

// This source code has been donated to the Public Domain.  Do
// whatever you want with it.

// A module to take care of all of bakeinflash's loaded fonts.
...
//		assert(0);
	...
	//	print_glyph(s_image_buffer, GLYPH_CACHE_TEXTURE_SIZE, GLYPH_CACHE_TEXTURE_SIZE);
		...
		image::alpha* im = image::create_alpha(GLYPH_CACHE_TEXTURE_SIZE, GLYPH_CACHE_TEXTURE_SIZE);
		memcpy(im->m_data, s_image_buffer, im->m_pitch * im->m_height);
		ge->m_bi = render::create_bitmap_info(im);

		// Fill in rendered_glyph_info.
		scale = version > 7 ? float(1024 * 20) : 1024;
...
//	}
	...
	glyph_provider_tu::~glyph_provider_tu()
	{
		...
		for (string_hash<glyph_array*>::iterator it = m_glyph.begin(); it != m_glyph.end(); ++it)
		{
			delete it->second;
		}
	}
...
/*
			...
			rect* bounds, float* advance)
	{
		...
		if (m_glyph.get(fontname, &ga))
		{
			...
			//	myprintf("glyph_provider_tu: finded %08X: %s\n", glyph_code, fontname.c_str());
				return ge.m_bi;
			}
		}
		else
		{
			// add new font
		//	myprintf("glyph_provider_tu: added font %s\n", fontname.c_str());
			ga = new glyph_array();
			m_glyph[fontname] = ga;
		}
	...
	//	myprintf("glyph_provider_tu: added %08X: %s\n", glyph_code, fontname.c_str());
		return ge.m_bi;
	}
