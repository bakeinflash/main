// bakeinflash_tools.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2004
...
namespace bakeinflash { namespace tools
{
	...
	struct copy_helper
	{
		...
		// Copy the data.  Return true on success, false on failure.
		{
			...
			{
				assert(0);
				myprintf("bakeinflash::tools::copy_helper() already done copy\n");
				return false;
			}
			...
			{
				myprintf("bakeinflash::tools::copy_helper asked to copy %d bytes\n",
					  bytes_to_copy);
				return false;
			}
		}
...
// Here's some code to compute that at run-time.
		...
		if (err != Z_OK)
		{
			...
			myprintf("write_placeholder_bitmap(): compress() failed.\n");
		}
		else
		{
...
}}	// end namespace bakeinflash::tools


int	bakeinflash::tools::process_swf(tu_file* swf_out, tu_file* in, const process_options& options)
{
	...
	// @@ TODO share this wrapper code somehow (also with bakeinflash_parser)
	...
	if (compressed)
	{
		...
		myprintf("bakeinflash can't read zipped SWF data; TU_CONFIG_LINK_TO_ZLIB is 0!\n");
		return -1;
#endif
	...
	if (options.m_zip_whole_file)
	{
		...
		myprintf("bakeinflash::tools::process_swf(): options.m_zip_whole_file is not implemented!  Output will not be zipped.\n");
	}
	...
	int	frame_count = 0;
	{
		...
		{
			// Error!
			myprintf("bakeinflash::tools::process_swf() -- unable to copy header data!\n");
			return 1;
		}
	}
...
//	IF_VERBOSE_PARSE(m_frame_size.print());
	...
	while ((Uint32) str.get_position() < file_end_pos)
	{
		...
		else
		{
			...
			{
				// Error!
				myprintf("bakeinflash::tools::process_swf() -- error copying tag!\n");
				return 1;
			}
		}
		...
		if (tag_type == 0)
		{
			if ((unsigned int) str.get_position() != file_end_pos)
			{
				...
				// movie.
				myprintf("warning: process_swf() hit stream-end tag, but not at the "
