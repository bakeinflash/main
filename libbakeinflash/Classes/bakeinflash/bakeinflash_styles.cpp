// bakeinflash_styles.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
namespace bakeinflash
{
	...
	void	fill_style::read(stream* in, int tag_type, movie_definition_sub* md)
	{
		...
		IF_VERBOSE_PARSE(myprintf("  fill_style read type = 0x%X\n", m_type));
		...
		if (m_type == 0x00)
		{
			...
			IF_VERBOSE_PARSE(myprintf("  color: ");
					 m_color.print());
		}
...
//			int num_gradients = in->read_uint(4);
			...
			IF_VERBOSE_PARSE(myprintf("  gradients: num_gradients = %d\n", num_gradients));
			...
			{
				m_gradient_bitmap_info = render::create_bitmap_info(NULL);
			}
		}
		else
		if (m_type == 0x13)
...
//			int gradient_count = in->read_uint(4);
		...
		if (m_type >= 0x40 && m_type <= 0x43)
		{
			...
			IF_VERBOSE_PARSE(myprintf("  bitmap_char = %d\n", bitmap_char_id));
		...
		else if (m_type == 0x12)
		{
			...
			for (int j = 0; j < im->m_height; j++)
			{
				for (int i = 0; i < im->m_width; i++)
				{
					...
					int	ratio = (int) floorf(255.5f * (float) sqrt(x * x + y * y));
					if (ratio > 255)
					{
						ratio = 255;
					}
	...
	// Push our style parameters into the renderer.
	{
		...
		else if (m_type == 0x10 || m_type == 0x12)
		{
			...
			if (m_gradient_bitmap_info != NULL)
			{
				render::fill_style_bitmap( fill_side, m_gradient_bitmap_info.get(), m_gradient_matrix,
	...
	bitmap_info*	fill_style::unzip_bitmap()
	{
		if (m_bitmap_character != NULL && m_bitmap_info == NULL)
		{
			// keep alive
			m_bitmap_info = m_bitmap_character->get_bitmap_info();
		}
		return m_bitmap_info.get();
	}

	void	fill_style::clear_bitmap()
	{
		if (m_bitmap_character != NULL && m_bitmap_info != NULL)
		{
			// hack
			int k = m_bitmap_info->get_ref_count();
			if (k == 2 && is_alive_bitmaps() == false)		// 2 = me + m_bitmap_character
			{
//				myprintf("clear_bitmap\n");
				m_bitmap_character->clear_bitmap_info();
				m_bitmap_info = NULL;
			}
		}
	}
	...
	void	line_style::apply(float ratio) const
	{
		...
		render::line_style_caps(m_caps_style);
	}
