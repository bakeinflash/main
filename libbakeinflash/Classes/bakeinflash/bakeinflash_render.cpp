// bakeinflash_render.cpp	-- Willem Kokke <willem@mindparity.com> 2003
...
namespace bakeinflash 
{
	...
	namespace render
	{
		...
		void clear()
		{
			if (s_render_handler) s_render_handler->clear();
		}

		void read_buffer(int x, int y, int w, int h, Uint8* buf)
		{
			if (s_render_handler) s_render_handler->read_buffer(x, y, w, h, buf);
		}
...
// 			else
// 			{
// 				myprintf("begin_display called, but no render_handler was registered by the app!\n");
// 			}
		}
		...
		{
			if (s_render_handler) s_render_handler->set_rgba(color);
		}
...
// 		};
		...
		{
			if (s_render_handler) s_render_handler->line_style_caps(caps_style);
		}
