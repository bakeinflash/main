...
int utf8::utf8to16(const char* szIn, wchar_t* wcOut, int nMax)
{
	mbstate_t   state = { 0 }; // Initial state
	size_t      nmbLen = 0, nwcLen = 0;
	wchar_t*    wcCur = wcOut;
	...
	{
		int rc = mbrtowc(wcCur, mbCur, 1, &state);
		switch (rc)
		{
			case 0:
			{  
...
//					printf("Currently in middle of mb conversion, state = %x\n", state);
			...
			default:
			{
				if (rc > 2) // The multibyte should never be larger than 2
				{
...
bool	utf8::is_unicode_character(const char* str)
{
	uint32	c = utf8::decode_next_unicode_character(&str);
	return c != INVALID;
}

void	utf8::encode_unicode_character(char* buffer, Uint32* index, Uint32 ucs_character)
{
