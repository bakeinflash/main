...
// Pull the paths into *tristate.
{
	...
	for (;;) {
		int coord_count = 0;
		int next_input_state = 0;
		const array<bakeinflash::point>& path_data = input->get_path_data(&coord_count, &next_input_state);
		// input_state:
		//   0: inside a path
		//   1: end of path
		//   2: end of input
...
//			assert((coord_count & 1) == 0);
			...
			for (int j = 0; j < coord_count; j++)
			{
				...
				ts->m_verts.push_back(poly_vert(path_data[j].m_x, path_data[j].m_y, vert_index - 1, vert_index + 1));
...
// data:
	array<coord_t>* m_results;
	const array<bakeinflash::path_part>& m_path_parts;
	int m_index;
	int m_style;

	ear_clip_array_io(array<coord_t>* results, const array<bakeinflash::path_part>&	paths, int index)	:
		m_results(results),
		m_path_parts(paths),
		m_index(index)
	{
		const bakeinflash::path_part* pp = &m_path_parts[index];
		m_style = pp->m_right_style;
	}
	...
	void get_estimated_path_data_size(int* estimated_path_count, int* estimated_coord_count)
	...
	// used to size array allocations.
	{
		...
		*estimated_coord_count = 0;
		for (int j = m_index, n = m_path_parts.size(); j < n; j++)
		{
			if (!m_path_parts[j].m_processed && m_path_parts[j].m_right_style == m_style && 
				m_path_parts[j].m_closed && m_path_parts[j].m_verts.size() > 0)
			{
				// Don't copy the end point, which should be a dupe of the begin point, since the
				// triangulator does that implicitly.
				*estimated_coord_count += m_path_parts[j].m_verts.size() - 1;
	...
	const array<bakeinflash::point>& get_path_data(int* coord_count, int* next_input_state)
	{
		...
		for (int j = m_index, n =  m_path_parts.size(); j < n; j++)
		{
			if (!m_path_parts[j].m_processed && m_path_parts[j].m_right_style == m_style && 
				m_path_parts[j].m_closed && m_path_parts[j].m_verts.size() > 0)
			{
//				m_path_parts[j].m_processed = true;
				bakeinflash::path_part* pj = const_cast<bakeinflash::path_part*>(&m_path_parts[j]);
				pj->m_processed = true;

				m_index++;

				*coord_count = m_path_parts[j].m_verts.size() - 1;
				*next_input_state = 1;
				return m_path_parts[j].m_verts;
			}
		}
...
static void compute_triangulation(
	...
	array<coord_t>* debug_edges)
{
	ear_clip_array_io<coord_t> io(results, path_parts, index);
	ear_clip_wrapper<coord_t, ear_clip_array_io<coord_t>, ear_clip_array_io<coord_t> >::
		compute_triangulation_impl(&io, &io, debug_halt_step, debug_edges);
}
