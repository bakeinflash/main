...
/*
void	zoom3(image_base* src, image_base* dst)
{
  typedef struct
  {
    Uint8 r;
  ...
  // Pointer setup
  rgb* sp = (rgb *) src->m_data;
	rgb* csp = sp;
  rgb* dp = (rgb *) dst->m_data;
  int sgap = src->m_pitch - src->m_width * sizeof(rgb);
  int dgap = dst->m_pitch - dst->m_width * sizeof(rgb);
  ...
  for (int y = 0; y < dst->m_height; y++)
    ...
    csy += sy;
	  ...
	  rgb* c01 = csp;
	  c01++;
	  rgb* c10 = (rgb *) ((Uint8 *) csp + src->m_pitch);
	  rgb* c11 = c10;
	  c11++;
	  ...
	  for (int x = 0; x < dst->m_width; x++)
	    ...
	    int csax = csx;
		  csx &= 0xffff;
			...
			// ABGR ordering. Interpolate colors
	    int ex = (csax & 0xffff);
	    int ey = (csay & 0xffff);
...
//	    dp->a = (((t2 - t1) * ey) >> 16) + t1;
	  ...
	  csp = (rgb *) ((Uint8 *) csp + (csy >> 16) * src->m_pitch);

  	// Advance destination pointers 
	  dp = (rgb *) ((Uint8 *) dp + dgap);
	}
}
*/
/*
void	zoom4(image_base* src, image_base* dst)
{
  typedef struct
  {
    Uint8 r;
  ...
  // Pointer setup
  rgba* sp = (rgba *) src->m_data;
	rgba* csp = sp;
  rgba* dp = (rgba *) dst->m_data;
  int sgap = src->m_pitch - src->m_width * sizeof(rgba);
  int dgap = dst->m_pitch - dst->m_width * sizeof(rgba);
  ...
  for (int y = 0; y < dst->m_height; y++)
    ...
    csy += sy;
	  ...
	  rgba* c01 = csp;
	  c01++;
	  rgba* c10 = (rgba *) ((Uint8 *) csp + src->m_pitch);
	  rgba* c11 = c10;
	  c11++;
	  ...
	  // Advance source pointer
	  csp = (rgba *) ((Uint8 *) csp + (csy >> 16) * src->m_pitch);
...
{
	if (src->m_type == image::image_base::RGB)
	{
		// rgb MUST BE converted to rgba because it must not overlap images om the scene
		typedef struct
		{
			Uint8 r;
			Uint8 g;
		...
		memset(dst->m_data, 0, dst->m_pitch * dst->m_height);
		const rgb* sp = (const rgb*) src->m_data;
		rgba* dp = (rgba*) dst->m_data;
		const Uint8* p = src->m_data;
	...
	if (src->m_type == image::image_base::RGBA)
	{
		memset(dst->m_data, 0, dst->m_pitch * dst->m_height);
		const Uint8* sp = src->m_data;
		Uint8* dp = dst->m_data;
		for (int y = 0; y < src->m_height; y++)
		{
			memcpy(dp, sp, src->m_pitch);
			dp += dst->m_pitch;
			sp += src->m_pitch;
		}
	}
