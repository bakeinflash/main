...
struct http_server
// Server class.  Listens to a socket, and depending on the query, dispatches requests to various registered handlers.
{
	http_server();
	~http_server();
	...
	{
		send_response(req, content_type, body.c_str(), body.size());
	}
...
private:
	// clients
	array<bakeinflash::tcp*> m_connection;
//data:
	net_interface* m_net;
	bakeinflash::tcp m_http;
