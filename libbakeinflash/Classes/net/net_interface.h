...
// Abstract IO stream interface.
{
	...
	virtual int read(char* data, int bytes, float timeout_seconds) = 0;
	...
	virtual int write(const char* data, int bytes, float timeout_seconds) = 0;

	int write_string(const char* str, float timeout_seconds)
	{
		return write(str, (int) strlen(str) + 1, timeout_seconds);
	}

	int write_string(const tu_string& str, float timeout_seconds)
	{
		return write_string(str.c_str(), timeout_seconds);
	}
};
