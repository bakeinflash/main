...
static tu_string s_proxy_addr;
static int s_proxy_port = 0;
const tu_string& get_proxy_addr()
{
	/*
	 if (s_proxy_addr.size() == 0)
	 {
	 s_proxy_port = 80;
...
int net_socket_tcp::read(char* data, int bytes, float timeout_seconds)
...
// number of bytes actually read.
{
	...
	for (;;)
	{
		...
		//		if (bytes_read > 0) { data[bytes_read] = 0; printf("%s", data); };
...
// closed or something.
{
	...
	for (;;)       
	{
		...
		{
			// Timeout?
			if (tu_timer::get_ticks() - start >= (uint32) timeout)
			{
				// Timed out.
				return 0;
			}
...
int net_socket_tcp::write(const char* data, int bytes, float timeout_seconds)
...
// bytes actually written.
{
	...
	for (;;)
	{
		...
		int rc = getsockopt (m_sock, SOL_SOCKET, SO_ERROR, (char*) (&error), &len);
	//	printf("%d %d\n", error, rc);
		if (error || rc)
		{
			closesocket(m_sock);
...
net_socket* net_interface_tcp::connect(const char* c_host, int port)
{
	if (c_host == NULL)
	{
		printf("net_interface_tcp, NULL url\n");
		return NULL;
	}
	if ( !(port > 0))
	{
		printf("net_interface_tcp, invalid port %d\n", port);
		return NULL;
	}
	...
	const char* host = get_proxy_port() > 0 ? get_proxy_addr().c_str() : c_host;
	...
	else
	{
		...
		if (he == NULL)
		{
			...
			fprintf(stderr, "host '%s' is unreachable, errcode=%d\n", host, er);
			closesocket(m_socket);
			m_socket = INVALID_SOCKET;
			return NULL;
		}
...
net_socket* net_interface_tcp::accept()
{
	...
	return new net_socket_tcp(remote_socket, NULL);
}
